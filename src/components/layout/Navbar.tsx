        ...
        <div className="hidden items-center gap-2 md:flex">
          ...
          <div className="relative" ref={menuRef}>
            ...
            >
              ...
              <span className="hidden text-sm font-medium lg:block">{user?.displayName || user?.username}</span>
            ...
            {menuOpen && (
              <div className="absolute left-0 mt-2 w-72 rounded-xl border border-subtle bg-surface p-3 shadow-elevated fade-in-up">
                <div className="flex items-center gap-3 border-b border-subtle pb-3">
                  <Avatar name={user?.displayName || user?.username} src={user?.avatarUrl} size={40} />
                  <div className="min-w-0 flex-1">
                    <div className="truncate text-sm font-semibold">{user?.displayName || user?.username}</div>
                    <div className="truncate text-xs text-neutral-500">{user?.email || `${user?.username}@local`}</div>
                  </div>
                </div>
                {capabilities && (
            ...
            <div className="mt-2 flex items-center justify-between border-t border-subtle pt-3">
              <div className="flex items-center gap-2">
                ...
                <div className="text-sm">
                  <div className="font-semibold leading-tight">{user?.displayName || user?.username}</div>
